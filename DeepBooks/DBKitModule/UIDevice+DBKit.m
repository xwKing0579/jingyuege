...
#import "UIDevice+DBKit.h"
#import <sys/utsname.h>
#import <SAMKeychain/SAMKeychain.h>


@implementation UIDevice (DBKit)

+ (NSString *)currentDeviceName{
    return DBSafeString(UIDevice.currentDevice.name);
}
...
+ (NSString *)systemVersion{
    return DBSafeString(UIDevice.currentDevice.systemVersion);
}

+ (NSString *)systemLanguage{
    return DBSafeString(NSLocale.preferredLanguages.firstObject);
}

+ (NSString *)deviceuuidString{
    NSString *service = UIApplication.appBundle;
    NSString *account = @"deviceIdentifierKey";
    NSString *uuidStr = [SAMKeychain passwordForService:service account:account];
    if (!uuidStr) {
        uuidStr = [[NSUUID UUID] UUIDString];
        [SAMKeychain setPassword:uuidStr forService:service account:account];
    }
    return uuidStr;
}


+ (NSString *)firstInitTimeKey{
    return [NSString stringWithFormat:@"%@%@%@",NSStringFromSelector(_cmd),UIApplication.appBundle,UIApplication.appBuild];
}

+ (double)firstInstallationTime{
    NSString *firstTime = [SAMKeychain passwordForService:UIApplication.appBundle account:self.firstInitTimeKey];
    if (firstTime.length > 0) return firstTime.doubleValue;
    return 0;
}

+ (void)saveFirstInstallationTime{
    if (self.firstInstallationTime > 0) return;
    
    double time = NSDate.date.timeIntervalSince1970;
    NSString *firstTime = [NSString stringWithFormat:@"%lf",time];
    [SAMKeychain setPassword:firstTime forService:UIApplication.appBundle account:self.firstInitTimeKey];
}

+ (BOOL)isVPNConnected {
    BOOL isVPN = NO;
    NSDictionary * proxySettings = (__bridge NSDictionary *)CFNetworkCopySystemProxySettings();
    NSArray *scoped = [proxySettings[@"__SCOPED__"] allKeys];
  
    for (NSString *key in scoped) {
        if ([key rangeOfString:@"ipsec"].location != NSNotFound ||
            [key rangeOfString:@"tun"].location != NSNotFound ||
            [key rangeOfString:@"tap"].location != NSNotFound ||
